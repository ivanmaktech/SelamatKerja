...
const Settings: React.FC<SettingsProps> = ({ user, onUpdateKakak, onUpdateEmployer }) => {
  ...
  const [kRestDays, setKRestDays] = useState(k?.restDays || 'weekly');
  const [kAccom, setKAccom] = useState(k?.accommodation || 'provided');
  const [kLang, setKLang] = useState(k?.language || 'Malay/Indonesian');
  ...
  const [eExp, setEExp] = useState(e?.yearsExperience || '1-3 years');
