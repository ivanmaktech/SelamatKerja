...
exports.evaluateContractFairness = exports.explainJobMatch = exports.processChatIntent = exports.answerRightsQuestion = exports.checkRecruitmentFee = exports.explainRawContract = exports.suggestContractRedFlags = exports.explainExtractedContract = exports.generateContractExplanation = void 0;
...
const groqClient = new openai_1.default({ apiKey: groqApiKey, baseURL: groqBaseUrl });
const groqModel = process.env.GROQ_MODEL || 'llama-3.1-8b-instant';
const runGroqPrompt = async (prompt) => {
...
const runWithFallback = async (label, prompt, fallback) => {
    let lastGeminiErr = null;
    // Try Gemini keys one by one
    for (let i = 0; i < geminiKeys.length; i++) {
        try {
            const ai = new genai_1.GoogleGenAI({ apiKey: geminiKeys[currentGeminiIndex] });
...
const buildContractFallbackSummary = (fields) => {
    const bullets = [];
    if (fields.salary)
        bullets.push(`Salary: ${fields.salary}`);
    if (fields.recruitment_fee)
        bullets.push(`Recruitment fee: ${fields.recruitment_fee}`);
    if (fields.deductions)
        bullets.push(`Deductions: ${fields.deductions}`);
    if (fields.working_hours)
        bullets.push(`Working hours: ${fields.working_hours}`);
    if (fields.rest_day)
        bullets.push(`Rest day or leave: ${fields.rest_day}`);
    if (fields.employment_duration)
        bullets.push(`Employment duration: ${fields.employment_duration}`);
    if (fields.termination_clause)
        bullets.push(`Termination: ${fields.termination_clause}`);
    if (fields.passport_clause)
        bullets.push(`Passport: ${fields.passport_clause}`);
    if (fields.accommodation)
        bullets.push(`Accommodation or food: ${fields.accommodation}`);
    if (fields.penalties)
        bullets.push(`Penalties: ${fields.penalties}`);
    if (!bullets.length) {
...
const buildContractSuggestionFallback = (fields) => {
...
const generateContractExplanation = async (contractText) => {
    const prompt = `
...
"""
    `;
    return runWithFallback('generateContractExplanation', prompt, () => "I am sorry, there was an issue understanding the contract. Please try again.");
};
exports.generateContractExplanation = generateContractExplanation;
const explainExtractedContract = async (fields) => {
    const prompt = `
...
exports.explainExtractedContract = explainExtractedContract;
const suggestContractRedFlags = async (fields) => {
    const prompt = `
...
exports.suggestContractRedFlags = suggestContractRedFlags;
const explainRawContract = async (contractText) => {
    const prompt = `
...
"""
    `;
    return runWithFallback('explainRawContract', prompt, () => 'I could not explain the contract right now. Please try a clearer image or paste the contract text.');
};
exports.explainRawContract = explainRawContract;
const checkRecruitmentFee = async (fee) => {
    const prompt = `
...
- Simple explanation of what they should ask next
    `;
    return runWithFallback('checkRecruitmentFee', prompt, () => "I am sorry, I could not verify this fee right now.");
};
exports.checkRecruitmentFee = checkRecruitmentFee;
const answerRightsQuestion = async (question) => {
    // 1. Retrieve knowledge from RAG
    const ragContext = await (0, ragService_1.queryKnowledgeBase)(question);
...
exports.answerRightsQuestion = answerRightsQuestion;
const processChatIntent = async (question) => {
    const prompt = `
...
exports.processChatIntent = processChatIntent;
const explainJobMatch = async (preferences, job, score) => {
    const prompt = `
...
exports.explainJobMatch = explainJobMatch;
const evaluateContractFairness = async (contract, preferences) => {
    const prompt = `
...
exports.evaluateContractFairness = evaluateContractFairness;
